...
import com.flir.flironegauravapplication.util.SystemUiHider;
...
import static com.flir.flironesdk.FrameProcessor.EMISSIVITY_MATTE;
import static com.flir.flironesdk.RenderedImage.ImageType.ThermalRGBA8888Image;
import static com.flir.flironesdk.RenderedImage.ImageType.ThermalRadiometricKelvinImage;
import static com.flir.flironesdk.RenderedImage.ImageType.VisibleAlignedRGBA8888Image;
...
public class GLPreviewActivity extends Activity implements Device.Delegate, FrameProcessor.Delegate, Device.StreamDelegate, Device.PowerUpdateDelegate {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RenderedImage.ImageType defaultImageType = ThermalRGBA8888Image;
        ...
        frameProcessor.setEmissivity((float) EMISSIVITY_MATTE);
    ...
    public void onFrameProcessed(final RenderedImage renderedImage) {
        ...
        if (renderedImage.imageType() == ThermalRadiometricKelvinImage) {
        ...
        */
            if (this.imageCaptureRequested) {
                Toast.makeText(GLPreviewActivity.this,
                        "Image Saved", Toast.LENGTH_LONG).show();
                short[] matThermalPixels = renderedImage.thermalPixelData();
                String a = matThermalPixels.toString();
                ByteBuffer.wrap(renderedImage.pixelData()).order(ByteOrder.LITTLE_ENDIAN).asShortBuffer().get(matThermalPixels);
            ...
            /*    int x =300;
                ...
                int step = renderedImage.width();
              *//* for(int x=0;x<100;x++){
                     for(int y=0;y<100;y++){
                short value = matThermalPixels[x + (y*step)];
...
*/
                ...
                new AsyncTask<Void, Void, Void>() {
                    ...
                    protected Void doInBackground(Void... params) {
                        ...
                        final String fileName = "FLIROne-Gaurav" + formatedDate + ".jpg";
    ...
    public void saveMatlab(final RenderedImage renderedImage) {
        ...
        String fileName = "FLIROne" + "" + ".jpg";
        String matFileName = "FLIROne" + "" + ".mat";
        String dataName = "FLIROne" + "";
        if (renderedImage.imageType() == RenderedImage.ImageType.ThermalRadiometricKelvinImage) {
            try {
                lastSavedPath = path + "" + matFileName;
                ...
                double[] dataCopy = new double[shortPixels.length];
                for (int i = 0; i < total; i++) {
                    //dataCopy[i] = (double) matThermalPixels[i];
                    dataCopy[i] = (double) shortPixels[i];
                }
                //MLDouble data = new MLDouble(dataName, dataCopy, height);
                MLDouble data = new MLDouble(dataName, dataCopy, 240);
                ArrayList<MLArray> allData = new ArrayList<>();
                allData.add(data);
                try {
                    mfw.write(new File(lastSavedPath), allData);
                    MediaScannerConnection.scanFile(context,
                            new String[]{path + "" + matFileName}, null,
                            new MediaScannerConnection.OnScanCompletedListener() {
                                @Override
                                public void onScanCompleted(String path, Uri uri) {
                                    Log.i("ExternalStorage", "Scanned " + path + ":");
    ...
    /*public void saveImage(final RenderedImage renderedImage) {
        if (renderedImage.imageType() == RenderedImage.ImageType.ThermalRadiometricKelvinImage) {
            ...
            if (this.imageCaptureRequested) {
                ...
                new Thread(new Runnable() {
                    public void run() {
                        ...
                        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ssZ", Locale.getDefault());
                        String formatedDate = sdf.format(new Date());
                        final String fileName = "FLIROne-kavi" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = path + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), frameProcessor);
